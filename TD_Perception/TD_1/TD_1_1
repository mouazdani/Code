...
pl.imshow(np.concatenate(images, axis = 0),'gray')
...
pad = np.zeros(images[0].shape)
line1 = images[:4]
line2 = images[4:]
line2.append(pad)
lines = np.concatenate([np.concatenate(line1, axis = 1), \
                        np.concatenate(line2, axis = 1)], axis = 0)
imshow(lines, 'gray')
...
###############################################
# Réponse : parce que tous les patterns ne font pas la même taille (et qu'ils doivent matcher pour pouvoir etre concatenes)
# (du coup, on recommente)
###############################################
...
patterns_size = [p.shape for p in patterns]
...
patterns_crop = [p.crop(0, 0, 480, 320) for p in patterns]
patterns_crop_size = [p.shape for p in patterns_crop]
###############################################

print patterns_crop_size

figure()
imshow(np.concatenate(patterns_crop, axis = 1),'gray')
...
#############################################################
# Question : combien de niveau contient un uint8 ?
#############################################################
# Reponse :
#
#############################################################
